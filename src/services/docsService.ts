...
export function buildApiDocs() {
  return {
    ...
    authentication: {
      ...
      exports: 'Protected feeds require PUBLIC_EXPORTS=true, x-export-token, or ?token=<export-token>. The x-export-token header takes precedence when both are supplied. Tokens with profileId or providerId are scoped to the matching profile/provider feed; profile-scoped tokens honor ExportProfile.rateLimit.'
    },
    endpoints: [
      ...
      { method: 'GET', path: '/api/admin/queue/summary', description: 'Admin/viewer queue health summary with pending, running, stale, and failed job counts.' },
      { method: 'DELETE', path: '/api/admin/queue?status=failed,completed&maxAgeHours=24', description: 'Admin cleanup action to remove completed and/or failed queue entries by status. maxAgeHours is optional and clears only older items.' },
      { method: 'POST', path: '/api/admin/queue/:id/retry', description: 'Admin retry for failed queued jobs. Reopens one additional attempt.' },
      { method: 'POST', path: '/api/admin/queue/stale/requeue', description: 'Admin recovery action that requeues running jobs whose worker lock expired.' },
      { method: 'GET', path: '/api/admin/audit', description: 'Admin audit log for source, import, profile, token, and channel mapping changes.' },
      { method: 'GET', path: '/api/admin/api-keys', description: 'Admin API key listing with masked key previews and bounded limit support.' },
      ...
      { method: 'GET', path: '/api/export-tokens', description: 'Admin export-token listing with masked token previews and bounded limit support.' },
      ...
      { method: 'GET', path: '/api/source-health/summary', description: 'Admin/viewer source health summary with latest status, failure streak, and backoff window.' }
    ]
  };
}
